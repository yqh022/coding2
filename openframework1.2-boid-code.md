...
Boid::Boid()
{
	...
	velocity = ofVec3f(ofRandom(-2, 2), ofRandom(0, 2));
}
...
void miniboid::draw(){
    ofSetColor(88, position.x, position.y);
...
void testApp::update(){
    ...
    ofVec3f min_a(0, 0);
    ofVec3f max_a(ofGetWidth(), ofGetHeight());
    
	for (int i = 0; i < boids.size(); i++)
	{
		boids[i]->update(boids, min, max);
	}
    for (int i = 0; i < boids_b.size(); i++)
    {
        boids_b[i]->update(boids_b, min_a, max_a);
    }
}
